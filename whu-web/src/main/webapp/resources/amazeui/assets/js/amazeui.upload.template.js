/**
 * @Date 2016-11-10
 * @Author xfworld AmazeUI Upload 模版
 * @param true:tableTemplate
 *            false:cardTemplate
 */
...
AmazeuiUploadDelegateTemplate.prototype.initTableTemplate = function() {
	var th = '<thead><tr><th>缩略图</th><th>文件信息</th><th>上传情况</th><th>上传状态</th><th>主页图片</th><th>栏目图片</th><th>附件信息</th><th>操作项</th></tr></thead><tbody></tbody>';
	$('#_template').append(th);
}
// 设置图片类型文件View模板
...
AmazeuiUploadDelegateTemplate.prototype.getTableRowTemplate = function() {
	var rowTemplate = '<tr>\
      ...
      <td>\
    	<input type="checkbox" value="" name="indexPic" class="indexPic">\
      </td>\
      <td>\
    	<input type="radio" value="" name="channelPicId">\
    	</td>\
      <td>\
    	<input type="checkbox" value="" name="isAttach" class="isAttach">\
    	<input type="hidden" name="aids" value="">\
    	</td>\
      </td>\
      <td class="am-text-middle"><button type="button" title="" class="am-btn am-btn-danger am-round am-btn-xs deleteAttach"><i class="am-icon-remove"></i>移除</button><button type="button" name="" oldName="" title="" isImg="" class="am-btn am-btn-primary am-round am-btn-xs insertAttach"><i class="am-icon-plus"></i> 插入</button></td>\
      </tr>';
	return rowTemplate;
}
// 创建默认模版行记录
