...
(function($) {
  ...
  AmazeuiUpload.prototype._defaultComplete = function(file) {
    ...
    if (result != null && result != undefined && result.status) {
      ...
      //TODO 自定义模版方法重写 Modified by cjbi on 2016/12/16 start
      this._defaultCompleteRow(file,result);
      //TODO 自定义模版方法重写 Modified by cjbi on 2016/12/16 end
  ...
  //TODO 自定义模版方法重写 Modified by cjbi on 2016/12/16 start
  AmazeuiUpload.prototype._defaultCompleteRow = function (file,result) {
	  var data = result.data;
	  console.log(data);
	  var uList = this._dynamicTemplate(file);
	  //如果是图片，就显示缩略图
	  if(data.isImg) {
		  uList.find('.image img').attr('src',contextPath+'/resources/upload/thumbnail/'+ data.newName);
		  uList.find('.insertAttach').attr({title: data.id, isImg: data.isImg, name: data.newName});
	  } else {
		  uList.find('.insertAttach').attr({title: data.id, isImg: data.isImg, name: data.newName,oldName: data.oldName});
	  }
	  uList.find('.indexPic').attr('value', data.id);
	  uList.find('.deleteAttach').attr('title', data.id);
	  uList.find('[name="channelPicId"]').attr('value', data.id);
	  uList.find('.isAttach').attr('value', data.id);
	  uList.find('[name="aids"]').attr('value', data.id);
  }
  //TODO 自定义模版方法重写 Modified by cjbi on 2016/12/16 end
